...
import { deleteFolderById } from 'apiSdk/folders';
...
function OrganizationViewPage() {
  ...
  const folderHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('folder', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/folders/create?organization_id=${data?.id}`}>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.folder?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/folders/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            <Td>
